...
if [ $oh_my_zsh = true ]; then
    ...
    if ! command -v python3 &>/dev/null; then
        ...
        sudo brew install -y python3
    fi

    # Install pip3
    if ! command -v pip3 &>/dev/null; then
        echo "Pip3 not found. Installing..."
        sudo brew install -y python3-pip
    fi
    ...
    if ! command -v chsh &>/dev/null; then
        ...
        sudo brew install util-linux-user
    fi

    # Install Zsh if not already present
    if ! command -v zsh &>/dev/null; then
        echo "Zsh not found. Installing..."
        sudo brew install -y zsh
    fi
...
if [ $tmux = true ]; then
    ...
    if ! command -v tmux &>/dev/null; then
        ...
        sudo brew install -y libevent
        sudo brew install -y ncurses
        sudo brew install -y perl
        ...
        if ! command -v libevent &>/dev/null || ! command -v ncurses &>/dev/null; then
            ...
            sudo brew install -y libevent-devel
            sudo brew install -y ncurses-devel
            sudo brew install -y gcc
            sudo brew install -y make
            sudo brew install -y bison
            #sudo yum install -y pkg-config
        fi
        sudo brew install -y autoconf
        sudo brew install -y automake
